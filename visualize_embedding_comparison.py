...
Generates graphs and visualizations for embedding model comparison results.
"""
...
# Color palette
COLORS = {
    'bge-small-en-v1.5': '#2E86AB',      # Blue Plot Color
    'all-MiniLM-L6-v2': '#A23B72',       # Purple Plot Color
    'all-mpnet-base-v2': '#F18F01',     # Orange Plot Color
    'TF-IDF': '#C73E1D',                 # Red Plot Color
}
...
# Create bars
bars1 = ax.bar(x - width/2, p10_values, width, label='Semantic Models',color=[COLORS.get(m, '#808080') for m in models], alpha=0.8)
bars2 = ax.bar(x + width/2, [tfidf_p10] * len(models), width,label='TF-IDF Baseline', color=COLORS['TF-IDF'], alpha=0.8)

# Add value labels on bars
for i, (bar, val) in enumerate(zip(bars1, p10_values)):
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2., height,f'{val:.1f}%', ha='center', va='bottom', fontweight='bold')

ax.text(bars2[0].get_x() + bars2[0].get_width()/2., tfidf_p10, f'{tfidf_p10:.1f}%', ha='center', va='bottom', fontweight='bold')

ax.set_xlabel('Embedding Model', fontsize=12, fontweight='bold')
ax.set_ylabel('Precision@10 (%)', fontsize=12, fontweight='bold')
ax.set_title('Precision@10 Comparison: Embedding Models vs TF-IDF Baseline',fontsize=14, fontweight='bold', pad=20)
ax.set_xticks(x)
...
bars1 = ax.bar(x - width/2, r10_values, width, label='Semantic Models', color=[COLORS.get(m, '#808080') for m in models], alpha=0.8)
bars2 = ax.bar(x + width/2, [tfidf_r10] * len(models), width, label='TF-IDF Baseline', color=COLORS['TF-IDF'], alpha=0.8)
...
ax.text(bars2[0].get_x() + bars2[0].get_width()/2., tfidf_r10,f'{tfidf_r10:.1f}%', ha='center', va='bottom', fontweight='bold')

ax.set_xlabel('Embedding Model', fontsize=12, fontweight='bold')
ax.set_ylabel('Recall@10 (%)', fontsize=12, fontweight='bold')
ax.set_title('Recall@10 Comparison: Embedding Models vs TF-IDF Baseline',fontsize=14, fontweight='bold', pad=20)
ax.set_xticks(x)
ax.set_xticklabels([m.replace('-', '\n') for m in models], fontsize=10, rotation=0)
ax.legend(loc='upper right', fontsize=11)
...
for model in results_df['model'].unique():
    ...
    ax1.plot(model_data['query'], model_data['semantic_P@10'] * 100, marker='o', label=model, linewidth=2, markersize=8, color=COLORS.get(model, '#808080'))

# Add TF-IDF baseline
tfidf_data = results_df[results_df['model'] == results_df['model'].iloc[0]]
ax1.plot(tfidf_data['query'], tfidf_data['tfidf_P@10'] * 100, marker='s', label='TF-IDF', linewidth=2, markersize=8, color=COLORS['TF-IDF'], linestyle='--', alpha=0.7)
...
for model in results_df['model'].unique():
    model_data = results_df[results_df['model'] == model]
    ax2.plot(model_data['query'], model_data['semantic_R@10'] * 100, marker='o', label=model, linewidth=2, markersize=8, color=COLORS.get(model, '#808080'))

ax2.plot(tfidf_data['query'], tfidf_data['tfidf_R@10'] * 100, marker='s', label='TF-IDF', linewidth=2, markersize=8, color=COLORS['TF-IDF'], linestyle='--', alpha=0.7)
...
# pivot table
pivot_p10 = results_df.pivot_table(values='semantic_P@10',index='query', columns='model') * 100

# Create heatmap
sns.heatmap(pivot_p10, annot=True, fmt='.1f', cmap='YlOrRd', cbar_kws={'label': 'Precision@10 (%)'}, linewidths=0.5, linecolor='gray', ax=ax)

ax.set_title('Precision@10 Heatmap: Models vs Queries', fontsize=14, fontweight='bold', pad=15)
ax.set_xlabel('Embedding Model', fontsize=12, fontweight='bold')
ax.set_ylabel('Query', fontsize=12, fontweight='bold')
ax.set_xticklabels([label.get_text().replace('-', '\n') for label in ax.get_xticklabels()], fontsize=9)
...
# pivot table
pivot_r10 = results_df.pivot_table(values='semantic_R@10', index='query', columns='model') * 100

# Create heatmap
sns.heatmap(pivot_r10, annot=True, fmt='.1f', cmap='YlGnBu', cbar_kws={'label': 'Recall@10 (%)'},linewidths=0.5, linecolor='gray', ax=ax)

ax.set_title('Recall@10 Heatmap: Models vs Queries', fontsize=14, fontweight='bold', pad=15)
ax.set_xlabel('Embedding Model', fontsize=12, fontweight='bold')
ax.set_ylabel('Query', fontsize=12, fontweight='bold')
ax.set_xticklabels([label.get_text().replace('-', '\n') for label in ax.get_xticklabels()],fontsize=9)
ax.set_yticklabels(ax.get_yticklabels(), rotation=0, fontsize=9)
...
x = np.arange(len(models))
bars = ax1.bar(x, summary_df['P@10_num'], color=[COLORS.get(m, '#808080') for m in models], alpha=0.8)
ax1.axhline(y=tfidf_p10, color=COLORS['TF-IDF'], linestyle='--', linewidth=2, label=f'TF-IDF ({tfidf_p10:.1f}%)')
for bar, val in zip(bars, summary_df['P@10_num']):
    ax1.text(bar.get_x() + bar.get_width()/2., bar.get_height(),f'{val:.1f}%', ha='center', va='bottom', fontsize=9, fontweight='bold')
...
bars = ax2.bar(x, summary_df['R@10_num'], color=[COLORS.get(m, '#808080') for m in models], alpha=0.8)
ax2.axhline(y=tfidf_r10, color=COLORS['TF-IDF'], linestyle='--', linewidth=2, label=f'TF-IDF ({tfidf_r10:.1f}%)')
for bar, val in zip(bars, summary_df['R@10_num']):
    ax2.text(bar.get_x() + bar.get_width()/2., bar.get_height(), f'{val:.1f}%', ha='center', va='bottom', fontsize=9, fontweight='bold')
...
bars1 = ax3.bar(x_pos - 0.2, summary_df['P_Improvement_num'], 0.4, label='P@10', color='#2E86AB', alpha=0.8)
bars2 = ax3.bar(x_pos + 0.2, summary_df['R_Improvement_num'], 0.4, label='R@10', color='#A23B72', alpha=0.8)
...
sns.heatmap(pivot_p10_short, annot=True, fmt='.1f', cmap='YlOrRd', cbar_kws={'label': 'Precision@10 (%)', 'shrink': 0.8},  linewidths=0.5, linecolor='gray', ax=ax4)
...
for model in results_df['model'].unique():
    model_data = results_df[results_df['model'] == model]
    ax5.plot(range(len(model_data)), model_data['semantic_P@10'] * 100, marker='o', label=model, linewidth=2, markersize=6, color=COLORS.get(model, '#808080'))
ax5.plot(range(len(tfidf_data)), tfidf_data['tfidf_P@10'] * 100, marker='s', label='TF-IDF', linewidth=2, markersize=6, color=COLORS['TF-IDF'], linestyle='--', alpha=0.7)
