#!/usr/bin/env python3
"""
Visualization of Recall Evaluation Results
"""
...
bars1 = ax1.bar(x - width/2, df['semantic_R@5']*100, width, label='Semantic Search',  color='#3498db', alpha=0.85)
bars2 = ax1.bar(x + width/2, df['tfidf_R@5']*100, width, label='TF-IDF Baseline', color='#e67e22', alpha=0.85)
...
for bars in [bars1, bars2]:
    for bar in bars:
        ...
        if height > 0:
            ax1.text(bar.get_x() + bar.get_width()/2., height + 0.2, f'{height:.1f}%', ha='center', va='bottom', fontsize=8)
...
bars1 = ax2.bar(x - width/2, df['semantic_R@10']*100, width, label='Semantic Search',  color='#2ecc71', alpha=0.85)
bars2 = ax2.bar(x + width/2, df['tfidf_R@10']*100, width, label='TF-IDF Baseline', color='#e74c3c', alpha=0.85)
...
for bars in [bars1, bars2]:
    for bar in bars:
        height = bar.get_height()
        if height > 0:
            ax2.text(bar.get_x() + bar.get_width()/2., height + 0.2,f'{height:.1f}%', ha='center', va='bottom', fontsize=8)
...
bars = ax3.bar(['Semantic\nSearch', 'TF-IDF\nBaseline'], [sem_r5_avg, tfidf_r5_avg],color=['#3498db', '#e67e22'], alpha=0.8, width=0.5)
...
for bar, val in zip(bars, [sem_r5_avg, tfidf_r5_avg]):
    height = bar.get_height()
    ax3.text(bar.get_x() + bar.get_width()/2., height + 0.2, f'{height:.2f}%', ha='center', va='bottom', fontweight='bold', fontsize=12)
...
bars = ax4.bar(['Semantic\nSearch', 'TF-IDF\nBaseline'], [sem_r10_avg, tfidf_r10_avg],color=['#2ecc71', '#e74c3c'], alpha=0.8, width=0.5)
...
for bar, val in zip(bars, [sem_r10_avg, tfidf_r10_avg]):
    height = bar.get_height()
    ax4.text(bar.get_x() + bar.get_width()/2., height + 0.2, f'{height:.2f}%', ha='center',va='bottom',fontweight='bold', fontsize=12)
...
for bar in bars:
    height = bar.get_height()
    ax5.text(bar.get_x() + bar.get_width()/2., height + 0.5,f'+{height:.1f}%', ha='center', va='bottom', fontweight='bold', fontsize=13)
...
### 6. Analysis for both Recall@5 and Recall@10
...
x = np.arange(3)
width = 0.35
bars1 = ax6.bar(x - width/2, [wins_r5, ties_r5, losses_r5], width, label='Recall@5', color=['#3498db', '#f39c12', '#e67e22'], alpha=0.8)
bars2 = ax6.bar(x + width/2, [wins_r10, ties_r10, losses_r10], width, label='Recall@10', color=['#2ecc71', '#f39c12', '#e74c3c'], alpha=0.8)
...
for bars in [bars1, bars2]:
    for bar in bars:
        height = bar.get_height()
        if height > 0:
            ax6.text(bar.get_x() + bar.get_width()/2., height + 0.1, f'{int(height)}', ha='center', va='bottom', fontweight='bold', fontsize=11)
